...
x_train, x_test, y_train, y_test = train_test_split(target,label,train_size=0.99,test_size=0.01)
    ...
    while run:
        for event in pygame.event.get():
            ...
            try:
                ...
                elif event.type == EVENT_DECIDE:
                    list_ =[[0 if blocks.grid[y][x] == 0 else 1 for x in range(10)] for y in range(20)]
                    curr = [int("".join([str(x) for x in row]), 2) for row in list_]
                    pred = knn.predict([curr])
                    if pred[0] == "D":
                        print(pred[0])
