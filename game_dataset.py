...
headers = ["X{}".format(x) for x in range(20)]
...
class BlocksGroup(pygame.sprite.OrderedUpdates):
    ...
    def update_current_block(self):
        try:
            ...
            move_list.append([[0 if self.grid[y][x] == 0 else 1 for x in range(10)] for y in range(20)])
            decisions.append('D')
        except BottomReached:
            self.stop_moving_current_block()
            self._create_new_block()
    ...
    def move_current_block(self):
        ...
        try:
            ...
            if self._current_block_movement_heading != pygame.K_DOWN:
                move_list.append([[0 if self.grid[y][x] == 0 else 1 for x in range(10)] for y in range(20)])
                decisions.append(arr[self._current_block_movement_heading])
        except BottomReached:
            self.stop_moving_current_block()
            self._create_new_block()
        else:
...
def write_csv():
        ...
        for move in move_list:
            move_bin.append([int("".join([str(x) for x in row]), 2) for row in move])
            move_bin[i].append(decisions[i])
            i += 1
        ...
        with open (filename, 'a') as csvfile:
            ...
            for i in range(len(move_bin)):
                writer.writerow(dict(zip(headers, move_bin[i])))
def main():
    pygame.init()
